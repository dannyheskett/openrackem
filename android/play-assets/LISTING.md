...
listing**, plus **Store settings** for category). Every image in this folder
is generated from code: `scripts/gen_store_assets.py` draws the icon set and
feature graphic (the 1-2-3 stepped-cards motif in the table palette), and
`scripts/gen_store_shots.c` renders the screenshots from real game states via
the portrait renderer, upscaled nearest-neighbor to the Play sizes.
...
Generated by `scripts/gen_store_shots.c` from deterministic game states
(menu, mid-round table, a placement, the round-scoring reveal), rendered by
the real portrait renderer at 540×960 and upscaled nearest-neighbor: 4×
1080×1920 in `screenshots/phone/`, the same frames at 2160×3840 in
`screenshots/tablet/` for both tablet slots.
