#!/usr/bin/env python3
"""Upload the openrackem app (.apk or .ipa) to AWS Device Farm and run a Fuzz
test on a real device, reporting pass/fail. Used by the devicefarm CI workflow.

openrackem is a native game with no tappable UI widgets, so a scripted
...
def main():
    ...
    run = df.schedule_run(
        projectArn=PROJECT,
        appArn=up["arn"],
        name=f"openrackem CI fuzz ({PLATFORM})",
        test=test_spec,
